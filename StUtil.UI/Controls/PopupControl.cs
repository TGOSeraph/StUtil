...
namespace StUtil.UI.Controls
{
    ...
    /// </summary>
    /// <remarks>
    /// Modified From: http://www.codeproject.com/Articles/17502/Simple-Popup-Control
    /// </remarks>
    public class PopupControl : ToolStripDropDown
    {
        ...
        private PopupControl _ownerPopup;
        private PopupControl _childPopup;
        ...
        protected override CreateParams CreateParams
        {
            ...
            get
            {
                ...
                cp.ExStyle |= NativeConsts.WS_EX_NOACTIVATE;
                if (NonInteractive) cp.ExStyle |= NativeConsts.WS_EX_TRANSPARENT | NativeConsts.WS_EX_LAYERED | NativeConsts.WS_EX_TOOLWINDOW;
                return cp;
            }
        }
        ...
        protected override void OnVisibleChanged(EventArgs e)
        {
            ...
            NativeEnums.AnimationFlags flags = Visible ? NativeEnums.AnimationFlags.Roll : NativeEnums.AnimationFlags.Hide;
            ...
            flags = flags | (NativeEnums.AnimationFlags.Mask & (NativeEnums.AnimationFlags)(int)_flags);
            NativeUtils.SetTopMost(this);
            NativeMethods.AnimateWindow(this.Handle, AnimationDuration, flags);
        }
        ...
        private void SetOwnerItem(Control control)
        {
            ...
            if (control is PopupControl)
            {
                PopupControl popupControl = control as PopupControl;
        ...
        {
           // if (!NativeConsts.IsRunningOnMono)
            {
                base.OnLayout(e);
                return;
            }
        ...
        {
            //if (m.Msg == NativeConsts.WM_PRINT && !Visible)
            //{
            //    Visible = true;
            //}
        ...
        {
            if (m.Msg == NativeConsts.WM_NCACTIVATE && m.WParam != IntPtr.Zero && _childPopup != null && _childPopup.Visible)
            {
                _childPopup.Hide();
            }
            ...
            if (m.Msg == NativeConsts.WM_NCHITTEST)
            {
                return OnNcHitTest(ref m, contentControl);
            }
            else if (m.Msg == NativeConsts.WM_GETMINMAXINFO)
            {
        ...
        {
            NativeStructs.MINMAXINFO minmax = (NativeStructs.MINMAXINFO)Marshal.PtrToStructure(m.LParam, typeof(NativeStructs.MINMAXINFO));
        ...
        private bool OnNcHitTest(ref Message m, bool contentControl)
        {
            if (NonInteractive)
            {
                m.Result = (IntPtr)NativeConsts.HTTRANSPARENT;
                return true;
            }

            int x = Cursor.Position.X; // NativeConsts.LOWORD(m.LParam);
            int y = Cursor.Position.Y; // NativeConsts.HIWORD(m.LParam);
            ...
            IntPtr transparent = new IntPtr(NativeConsts.HTTRANSPARENT);

            if (_resizableTop)
            {
                if (_resizableLeft && gripBouns.TopLeft.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTTOPLEFT;
                    return true;
                }
                if (!_resizableLeft && gripBouns.TopRight.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTTOPRIGHT;
                    return true;
                }
                if (gripBouns.Top.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTTOP;
                    return true;
                }
            }
            else
            {
                ...
                {
                    m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTBOTTOMLEFT;
                    return true;
                }
                if (!_resizableLeft && gripBouns.BottomRight.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTBOTTOMRIGHT;
                    return true;
                }
                if (gripBouns.Bottom.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTBOTTOM;
                    return true;
                }
            }
            if (_resizableLeft && gripBouns.Left.Contains(clientLocation))
            {
                m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTLEFT;
                return true;
            }
            if (!_resizableLeft && gripBouns.Right.Contains(clientLocation))
            {
                m.Result = contentControl ? transparent : (IntPtr)NativeConsts.HTRIGHT;
                return true;
            }
            return false;
        }
